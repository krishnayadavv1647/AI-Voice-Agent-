...
export default function Welcome() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-neutral-50 text-ink">
      ...
      <header className="sticky top-0 z-20 border-b border-hairline/70 bg-white/85 backdrop-blur">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          <Link to="/" className="flex min-w-0 items-center gap-3">
            <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-ink text-white shadow-soft">
              <Headphones size={22} />
            </span>
            <span className="min-w-0">
              <span className="block truncate text-lg font-semibold tracking-tight">AI Voice Agent Platform</span>
              <span className="block truncate text-xs text-neutral-500">Independent AI calling platform</span>
            </span>
          </Link>
      ...
      <section className="border-y border-hairline bg-white/80 px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto w-full max-w-7xl">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {problems.map((problem) => (
              <div key={problem} className="rounded-2xl border border-hairline bg-neutral-50 p-5">
                <CheckCircle2 className="mb-4 text-rose-500" size={21} />
                <p className="font-semibold text-neutral-800">{problem}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto w-full max-w-7xl">
          ...
          <div className="mt-8 grid gap-4 lg:grid-cols-3">
            {[
              ...
              <article key={number} className="rounded-2xl border border-hairline bg-white p-6 shadow-sm">
        ...
        <div className="mx-auto w-full max-w-4xl">
          ...
          <div className="mt-8 grid gap-4">
            {faqs.map(([question, answer]) => (
              <article key={question} className="rounded-2xl border border-hairline bg-neutral-50 p-5">
      ...
      <section className="px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto w-full max-w-7xl rounded-[2rem] border border-hairline bg-white p-6 text-center shadow-soft sm:p-10">
          <Sparkles className="mx-auto mb-4 text-brand-600" size={28} />
          <h2 className="text-3xl font-black tracking-tight text-ink">Start Building Your AI Voice Agent Today</h2>
...
function SectionHeader({ eyebrow, title, text, dark = false }) {
  return (
    <div className="min-w-0">
      ...
      <h2 className={`mt-3 max-w-3xl text-3xl font-black tracking-tight sm:text-4xl ${dark ? "text-white" : "text-ink"}`}>{title}</h2>
      {text && <p className={`mt-3 max-w-2xl text-sm leading-6 ${dark ? "text-neutral-300" : "text-neutral-600"}`}>{text}</p>}
    </div>
  );
}
...
function InfoCard({ title, text, icon: Icon, compact = false }) {
  return (
    <article className="rounded-2xl border border-hairline bg-white p-5 shadow-sm">
      <div className="mb-4 grid h-11 w-11 place-items-center rounded-2xl bg-brand-50 text-brand-700">
        <Icon size={21} />
      </div>
      <h3 className="text-base font-semibold text-ink">{title}</h3>
      <p className={`mt-2 text-sm leading-6 text-neutral-600 ${compact ? "line-clamp-none" : ""}`}>{text}</p>
    </article>
  );
}
...
function HeroMockup() {
  return (
    <div className="min-w-0 rounded-[2rem] border border-hairline bg-white p-4 shadow-soft">
      <div className="rounded-[1.5rem] bg-ink p-4 text-white">
        ...
        <div className="grid gap-3 md:grid-cols-[1fr_0.85fr]">
          <div className="rounded-2xl bg-white p-4 text-ink">
            <p className="text-xs font-bold uppercase tracking-wide text-neutral-500">AI agent response</p>
            <p className="mt-2 text-sm leading-6">“Sure, I can help book an appointment. May I know your name and preferred time?”</p>
            <div className="mt-4 grid gap-2 text-sm">
              <div className="rounded-xl bg-neutral-50 px-3 py-2"><strong>Lead:</strong> Rahul Sharma</div>
              <div className="rounded-xl bg-neutral-50 px-3 py-2"><strong>Intent:</strong> Appointment booking</div>
              <div className="rounded-xl bg-neutral-50 px-3 py-2"><strong>Status:</strong> Qualified</div>
            </div>
          </div>
          <div className="grid gap-3">
            <div className="rounded-2xl border border-white/10 bg-white/10 p-4">
              <p className="mb-3 text-xs font-bold uppercase tracking-wide text-neutral-300">Workflow nodes</p>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/10 p-4">
              <p className="mb-3 text-xs font-bold uppercase tracking-wide text-neutral-300">Providers</p>
              <div className="flex flex-wrap gap-2">
                {["Twilio", "Exotel", "Vonage"].map((name) => (
                  <span key={name} className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-neutral-800">{name}</span>
                ))}
              </div>
            </div>
          </div>
    ...
    <div className="mt-8 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
      <div className="rounded-2xl border border-hairline bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between gap-3">
          <h3 className="font-semibold text-ink">Agent list</h3>
          <span className="badge bg-emerald-50 text-emerald-700">3 Active</span>
        </div>
        {["Reception Agent", "Sales Agent", "Support Agent"].map((agent) => (
          <div key={agent} className="mb-3 flex items-center justify-between gap-3 rounded-2xl bg-neutral-50 p-3 last:mb-0">
            <span className="font-semibold text-neutral-800">{agent}</span>
      ...
      <div className="grid gap-4">
        <div className="rounded-2xl border border-hairline bg-white p-5 shadow-sm">
          <h3 className="mb-4 font-semibold text-ink">Telephony config</h3>
          <div className="flex flex-wrap gap-2">
        ...
        <div className="rounded-2xl border border-hairline bg-white p-5 shadow-sm">
          <h3 className="mb-4 font-semibold text-ink">Lead table</h3>
          <div className="grid gap-2 text-sm text-neutral-600">
            <div className="grid grid-cols-3 gap-2 rounded-xl bg-neutral-50 p-2"><span>Rahul</span><span>Booked</span><span>2m call</span></div>
            <div className="grid grid-cols-3 gap-2 rounded-xl bg-neutral-50 p-2"><span>Priya</span><span>New</span><span>Callback</span></div>
          </div>
        </div>
      </div>
    </div>
