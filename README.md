...
## Suggested Authentication Routes
...
## Suggested Dograh Routes

```text
GET    /api/integrations/dograh/status
POST   /api/integrations/dograh/connect
POST   /api/integrations/dograh/test
PUT    /api/integrations/dograh
DELETE /api/integrations/dograh
```

## Suggested Call Routes
...
{
  "success": false,
  "message": "Unable to update the Dograh workflow",
  "error": "DOGRAH_WORKFLOW_UPDATE_FAILED"
}
```
...
- Frontend URL is not allowed.
- Backend URL is incorrect.
- HTTP and HTTPS are mixed.
- The domain contains an incorrect trailing slash.
- Production frontend URL is missing from allowed origins.
...
Make sure the complete application is wrapped:
...
- Dograh node configuration
- Output audio format

## 5. Agent Cannot Hear the User
...
## 6. Editing Creates a New Dograh Agent or Workflow

Check:

- `dograhWorkflowId` is saved in the database.
- The edit API sends the existing agent ID.
- The backend loads the existing agent.
- The workflow update method uses the saved workflow ID.
- The create workflow method is not called unconditionally.

## 7. Dograh Shows Connected but Is Not Actually Connected

Remove hardcoded status from the frontend.
...
- The Dograh API is reachable.

## 8. `Cannot GET /`

This usually means the Express backend is running but no route exists for `/`.
...
Add a root route only when required:

```js
app.get("/", (req, res) => {
  res.json({ message: "AI Voice Agent API is running" });
});
```

## 9. Invalid or Expired API Key
...
4. Restart the backend when environment variables changed.
5. Never log the full key.
...
Confirm the actual command inside `backend/package.json`.
...
- Dograh connection works
- Agent creation works
- Agent update does not create duplicates
- Lead creation works
- Calling works
- Webhooks work
...
- Encrypt user API keys before saving them.
- Never send secret keys back to the frontend.
- Verify ownership of every agent, lead, call, and integration.
- Add rate limiting.
- Validate all request data.
- Sanitize user input.
- Use secure password hashing.
- Use HTTP-only secure cookies when suitable.
- Restrict CORS.
- Add audit logs.
- Rotate compromised keys.
- Disable global provider fallback by default.
- Verify webhook signatures.
- Do not expose raw backend errors in production.
- Add request timeouts for external APIs.
- Add retry limits to prevent duplicate calls or workflows.

---

# Recommended User Workflow

For the best result, use the application in this order:
...
5. Connect voice provider
6. Connect transcription provider
7. Connect telephony provider
8. Create agent
9. Test agent through web call
10. Add lead
11. Start one test phone call
12. Review result
13. Improve prompt
14. Retest
15. Start production calls
```

Do not begin bulk calling before testing the agent with multiple realistic conversations.
...
- Replace placeholder URLs with real project URLs.
- Replace suggested routes with the routes used in the codebase.
- Confirm all environment variable names from the source code.
- Confirm all commands from `package.json`.
- Remove sections for features that are not yet implemented.
- Do not display fake integration status.
- Do not create a new Dograh workflow when editing an existing agent.
...
- API endpoint
- Request payload without secret keys
